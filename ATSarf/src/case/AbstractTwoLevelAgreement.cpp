...
AbstractTwoLevelAgreement::AbstractTwoLevelAgreement(QString *text, QString fileName, AbstractGraph *generatedGraph,
    ...
    segmentationRecall = segmentationPrecision = underSegmentationRatio = nameRecall = namePrecision = boundaryRecall =
        boundaryPrecision = underNameRecall = underNamePrecision = underBoundaryRecall = underBoundaryPrecision = 0;
}

int AbstractTwoLevelAgreement::commonNames(const SelectionList &list1, const SelectionList &list2,
                                           QSet<int> &visitedTags, int &allCommon) {
    return ::commonNames<AbstractTwoLevelAgreement>(text, list1, list2, visitedTags, allCommon, *this);
}

void AbstractTwoLevelAgreement::overLapNamesFinished(const SelectionList &tagWords, const SelectionList &outputWords,
                                                     int &numWords) {
...
void AbstractTwoLevelAgreement::overLapMainFinished(int i, int j, const SelectionList &tagNames,
                                                    const SelectionList &outputNames, int &numNames) {
    ...
    if (countCorrect > 0) {
        ...
        while (k < tagNames.size() && h < outputNames.size()) {
            int start1 = tagNames[k].first, end1 = tagNames[k].second, start2 = outputNames[h].first,
                end2 = outputNames[h].second;
            if (overLaps(start1, end1, start2, end2) && start1 != end2) {
                ...
                if (!foundH) { // common_i and common_j now are not same size, bc recall and precision need different
    ...
    if (underComputation) { // under computation only
        if (hadithParameters.detailed_statistics) {
            theSarf->displayed_error << /*text->mid(start1,end1-start1+1)*/ i << "\t"
                                     << /*text->mid(start2,end2-start2+1)*/ j << "\t" << countCommon << "/"
                                     << countCorrect << "\t" << allCommonCount << "/" << countDetected << "\n";
        }
    }
}
...
bool AbstractTwoLevelAgreement::readAnnotation(QString tagFileName, OutputDataList &tags, AbstractGraph *globalGraph) {
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        tags.readFromStream(
            in, dummyLocalGraph); // HadithChainGraph provided but will not be in use after call just duplicated
...
int AbstractTwoLevelAgreement::calculateStatisticsHelper() {
    ...
    while (i < tags.size() && j < outputList.size()) {
        int start1 = tags[i].getMainStart(), end1 = tags[i].getMainEnd(), start2 = outputList[j].getMainStart(),
            end2 = outputList[j].getMainEnd();
        if (overLaps(start1, end1, start2, end2) && start1 != end2) {
            const SelectionList &tagNames = tags[i].getNamesList(), &outputNames = outputList[j].getNamesList();
            ...
            if (!foundJ) { // common_i and common_j now are not same size, bc recall and precision need different
                           // treatment for overlap
                common_j.append(j);
            ...
            if (hadithParameters.detailed_statistics) {
                theSarf->displayed_error << "-----\t" << /*text->mid(start2,end2-start2+1)*/ j << "\n";
            }
            //[max-boundary computations
            overLapMainFinished(-1, -1, tagNamesOverLap, outputNamesOverLap, numNames);
            //]
