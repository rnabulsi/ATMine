...
int insert_NProp(QString word, QList<long> *abstract_categories, int source_id, QString description = "Proper noun") {
    ...
    // insert word as is
    int stem_id = insert_item(STEM, removeDiacritics(word), word, (hasAL ? "Nprop_Al" : "Nprop_Al"), source_id,
                              abstract_categories, description, "", "", "");
    if (stem_id < 0) {
    ...
    stem_id =
        insert_item(STEM, removeDiacritics(possessive), possessive, (possessive.startsWith(lam) ? "Nall_L" : "Nall"),
                    source_id, abstract_categories, QString("possessive form of ").append(description), "", "", "");
    if (stem_id < 0) {
...
int insertRuleAccordingToExpression(item_types type, QString cat1, QString cat2, QString resCat, QString inflections,
                                    int source_id) {
    rules rule = (type == PREFIX ? AA : CC);
    QList<long> catIds1, catIds2;
    bool exp1 = cat1.startsWith("{") && cat1.endsWith("}"), exp2 = cat2.startsWith("{") && cat2.endsWith("}"),
...
int insert_buckwalter() {
    int source_id = insert_source("Buckwalter Dictionaries",
                                  "modifying aramorph.pl + insert_buckwalter() c++ code fragment", "Jad Makhlouta");
    // items
    ...
    for (int j = 0; j < num_files_items; j++) {
        ...
        while (!file.atEnd()) {
            ...
            // 3 letters are not properly transfered using perl script because it is not based on unicode, here we solve
            // them manually
            ...
            if (types[j] == STEM) {
                // if (!POS.contains("+"))//choose what to do with POS like "litaw~i/ADV+hi/PRON_3MS"
                {
                    ...
                    if (abstract_list.size() >= 2)
                        ...
                        abstract_category = "";
                        theSarf->out << "Unexpected Error: split on " << POS << " " << abstract_list.size() << " "
                                     << abstract_list.at(0);
                        return 1;
                    }
            ...
            if (insert_item(types[j], item, raw_data, category, source_id, abstract_categories, description, POS, "",
                            lemmaID) < 0) {
        ...
        theSarf->out << QString("\nSuccessfully processed all %1 %2 entries\n").arg(line_num).arg(
            interpret_type(types[j]));
        input.close();
    }
    // compatibility rules
    const int num_files_rules = 5;
    const QString rules_files[num_files_rules] = {folder + "tableAA" + tag, folder + "tableCC" + tag,
                                                  folder + "tableAB", folder + "tableBC", folder + "tableAC"};
    ...
    for (int j = 0; j < num_files_rules; j++) {
        ...
        while (!file.atEnd()) {
            ...
            //#ifdef INSERT_ONLY_SUFFIXES
                ...
                assert(insert_category(cat1, (rule[j] == AB || rule[j] == AC ? PREFIX : STEM), source_id, false) >=
                       0); // if not AB or AC => BC
                assert(insert_category(cat2, (rule[j] == AC || rule[j] == BC ? SUFFIX : STEM), source_id, false) >=
                       0); ////if not AC or BC => AB
            }
            //#endif
...
int insert_propernames() {
    ...
    long abstract_Noun_Prop_id =
        insert_category("NOUN_PROP", STEM, dbitvec(max_sources), true); // returns id if already present
    long abstract_people_names =
        insert_category("Name of Person", STEM, dbitvec(max_sources), true); // returns id if already present
    long abstract_male_names =
        insert_category("Male Names", STEM, dbitvec(max_sources), true); // returns id if already present
    long abstract_female_names =
        insert_category("Female Names", STEM, dbitvec(max_sources), true); // returns id if already present
    QString file_name;
...
int insert_placenames() // not yet complete
{
    ...
    int folders_source_id = insert_source(
        "ar.wikipedia.org/",
        "direct copy from html and dividing them into folders and subfiles according to continents and cities/towns",
        "Hamza Harkous");

    long abstract_Noun_Prop_id =
        insert_category("NOUN_PROP", STEM, dbitvec(max_sources), true); ////returns id if already present
    long abstract_place_names =
        insert_category("Name of Place", STEM, dbitvec(max_sources), true); ////returns id if already present
    long abstract_continent_name =
        insert_category("Continent", STEM, dbitvec(max_sources), true); ////returns id if already present
    long abstract_country_name =
        insert_category("Country", STEM, dbitvec(max_sources), true); ////returns id if already present
    long abstract_city_name =
        insert_category("City/Town", STEM, dbitvec(max_sources), true); ////returns id if already present
    QString file_name;
    ...
    foreach (file_name, folder.entryList()) {
        if (file_name.endsWith(".txt")) {
            ...
            while (!file.atEnd()) {
                ...
                if (insert_NProp(line, abstract_categories, source_id, file_name.split(".").at(0)) < 0) { /*return -1*/
                    ;
                }
            }
            theSarf->out << QString("\nSuccessfully processed all %1 %2 entries\n").arg(line_num).arg(file_name);
        ...
        } else if (!file_name.contains('.')) // assuming this means folder
        {
            ...
            if (continent.count() > 1) {
                ...
                abstract_categories->append(abstract_place_names);
                if (insert_NProp(continent_arabic, abstract_categories, folders_source_id, continent_english) <
                    0) { /*return -1*/
                    ;
                }
            }
            ...
            foreach (file_name2, folder2.entryList()) {
                ...
                if (country.count() > 1) {
                    ...
                    if (insert_NProp(country_arabic, abstract_categories, folders_source_id, country_english) <
                        0) { /*return -1*/
                        ;
                    }
                }
                ...
                while (!file.atEnd()) {
                    ...
                    if (line.isEmpty()) // ignore empty lines if they exist
                        continue;
                    if (line.split(" ")[0].length() == 1) // ignore lines that are just for sorting in alphabatical
                                                          // order
                        continue;
                    QList<long> *abstract_categories = new QList<long>();
                    abstract_categories->append(abstract_Noun_Prop_id);
                    ...
                    if (city.count() > 1) // add all the text in addition to the primary part
                        if (insert_NProp(alltext, abstract_categories, folders_source_id,
                                         "city/town in " + country_english) < 0) { /*return -1*/
                            ;
                        }
                    if (insert_NProp(city[0], abstract_categories, folders_source_id,
                                     "city/town in " + country_english) < 0) { /*return -1*/
                        ;
                    }
                }
...
int insert_time_categorizations() {
    ...
    while (!file.atEnd()) {
        ...
        if (line.isEmpty()) // ignore empty lines if they exist
            continue;
        QList<long> *abstract_categories = new QList<long>();
        long time_abstract_category_id = insert_category("Time", STEM, source_id, true); // returns id if already
                                                                                         // present
        ...
        long cateorized_abstract_category_id =
            insert_category(cateorized_abstract_category, STEM, source_id, true); // returns id if already present
