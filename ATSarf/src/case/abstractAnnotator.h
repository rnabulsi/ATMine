...
public:
    ...
    inline static bool isConsistentWithSelectionCondidtion(int start, int end, int tagStart, int tagEnd,
                                                           SelectionMode selectionMode = SELECTION_OUTSIDE) {
        bool con1 = selectionMode == SELECTION_OUTSIDE && start <= tagStart && end >= tagEnd,
             ...
             con3 = selectionMode == SELECTION_OUTSIDEOVERLAP &&
                    ((start <= tagStart && end >= tagStart) || (start <= tagEnd && end >= tagEnd));
        return con1 || con2 || con3;
    }
    int findSelection(int startIndex = 0,
                      SelectionMode selectionMode = SELECTION_OUTSIDE); // based on user text selection
