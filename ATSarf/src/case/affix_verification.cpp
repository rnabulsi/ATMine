...
void drawAffixGraph(item_types type) {
    ...
    QQueue<node *> queue;
    node *base =
        (type == PREFIX ? database_info.Prefix_Tree->getFirstNode() : database_info.Suffix_Tree->getFirstNode());
    ...
    while (!queue.isEmpty()) {
        ...
        if (n.isLetterNode()) {
            ...
            d_out << "n" << getGraphID(map, last_id, &n) << " [label=\"" << (letter != '\0' ? letter : '-')
                  << "\", shape=circle];\n";
        } else {
            ...
            for (int i = 0; i < size; i++) {
                d_out << "r" << getGraphID(map, last_id, &n) << "_" << i << " [label=\"" << r.raw_datas[i].getActual()
            ...
            if (nc != NULL) {
                ...
                if (n.isLetterNode())
                    ...
                    for (int i = 0; i < l.size(); i++) {
                        d_out << "r" << getGraphID(map, last_id, rn) << "_" << i << "->"
                              << "n" << getGraphID(map, last_id, nc) << ";\n";
                    }
                }
#else
    ...
    try {
        ...
        mw->show();
#else
        system(QString("kgraphviewer ./%1.dot -caption \"%1.dot\" &")
                   .arg(t)
